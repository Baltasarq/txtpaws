...
const std::string caracteresExtrasIds = "._����������������";
...
bool ParseDefs::compruebaId(const std::string &x)
{
    ...
    if ( isalpha( x[0] ) )
    {
            for(; i < tam; ++i)
            {
                    // Un id puede contener '_', letra o d�gito
                    if ( !isdigit( x[i] )
                      && !isalpha( x[i] )
...
/*
	El preprocesador procesa las �rdens #ifdef ... #else ... #endif,
	actuando en consecuencia.
	Se trata de una peque�a m�quina de estados. Como los #if ...
...
void ParseDefs::procEntrada()
{
    ...
    while ( !( linact.empty() ) ) {
      ...
      // Pasar las l�neas en blanco tal y como est�n
      ...
      if ( !( esContinuacionMacro( linact ) ) )
      {
          ...
          if ( getEstadoPreProc() != EN_IF_TRAGA )
          {
               ...
               if ( cambiaEstado(linact) )
               {
                      ...
                      // Si estamos en modo de debug, a�adir el n�mero de l�nea
                      if ( esModoDebug() ) {
        ...
        && devEstado() != SYSMSG )
      {
        linact += infoDebug.str();
          ...
          // Escribir la l�nea ya procesada en la salida
    ...
    // Volcados de informaci�n varios
    if ( log != NULL ) {
            ts->dumpTxt( *log );
    }
...
/*
	...
	A�ade el nombre del fichero a todas las rutas almacenadas,
...
/*
	Procesa todas las opciones que hayan sido incluidas en la l�nea de comando,
...
*/
{
	...
	while ( argAct < argc )
	{
		...
		if ( arg[0] == '-' )
		{
			...
			{
				// EL modo de depuraci�n hace que se a�ada
...
// ------------------------------------------- Dependientes de la implementaci�n
#ifdef __TEXT__UI
